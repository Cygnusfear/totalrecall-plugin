...
export class PostgresSynthesisDatabase implements ISynthesisDatabase {
    ...
    for (const { location, column, priority } of TEXT_SEARCH_FIELDS) {
      ...
      if (nodeTypes?.length) {
        rows = await this.sql`
          SELECT ${this.sql(this.nodeColumns)}
          FROM synthesis_nodes
          WHERE ${this.sql(column)} ILIKE ${searchPattern}
      ...
      for (const row of rows) {
        const node = this.mapNodeFromDb(row as Record<string, unknown>);
        if (!seenIds.has(node.id)) {
          seenIds.add(node.id);
          results.push({ ...node, match_location: location, priority });
        }
      }
    }
  ...
  async queryNodesByTimeRange(
    ...
    const query = this.sql`
      SELECT ${this.sql(this.nodeColumns)}
      FROM synthesis_nodes
      WHERE ${this.sql(orderBy)} BETWEEN ${startTime} AND ${endTime}
      ${nodeTypes && nodeTypes.length > 0 ? this.sql`AND node_type = ANY(${nodeTypes})` : this.sql``}
      ORDER BY ${this.sql(orderBy)} DESC
      LIMIT ${limit}
    `;

    const rows = await query;
    return rows.map((row) => this.mapNodeFromDb(row));
  }
